...
class DbLogAdapter extends \Zend\Db\TableGateway\TableGateway implements \BoilerAppLogger\Service\Adapter\LogAdapterInterface{
	/**
	 * @var int
	 */
	protected $logId;
	 ...
	 * @return \BoilerAppLogger\Service\Adapter\DbLogAdapter
	 */
	...
	/**
	 * @see \BoilerAppLogger\Service\Adapter\LogAdapterInterface::getLogId()
	 * @return int
	 */
	 ...
	 * @param mixed $oParam
	 * @see \BoilerAppLogger\Service\Adapter\LogAdapterInterface::log()
	 * @return \BoilerAppLogger\Service\Adapter\DbLogAdapter
	 */
	 ...
	 * @see \BoilerAppLogger\Service\Adapter\LogAdapterInterface::completed()
	 * @return \BoilerAppLogger\Service\Adapter\DbLogAdapter
	 */
		...
		if(!isset(self::$logs[$this->logId]))throw new \Exception('Log "'.$this->logId.'" is not initialized');
			...
			if(!empty($aEntities))foreach($aEntities as $aEntity){
				...
				switch($aEntity['entity_action']){
					case \BoilerAppLogger\Service\LoggerService::LOG_EVENT_CREATE_ENTITY:
					...
					case \BoilerAppLogger\Service\LoggerService::LOG_EVENT_DELETE_ENTITY:
						$oStatement = $this->sql->update()
						->set(array('entity_log_id' => $sRealLogId))
					...
					case \BoilerAppLogger\Service\LoggerService::LOG_EVENT_UPDATE_ENTITY:
