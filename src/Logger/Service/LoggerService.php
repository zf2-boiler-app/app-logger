<?php
namespace BoilerAppLogger\Service;
class LoggerService implements \Zend\EventManager\SharedEventManagerAwareInterface{
	 ...
	 * @return \BoilerAppLogger\Service\LoggerService
	 */
	...
	*/
	 ...
	 * @param string|\BoilerAppLogger\Service\Adapter\LogAdapterInterface $oAdapter
	 * @throws \Exception
	 * @return \BoilerAppLogger\Service\LoggerService
	 */
	public function setAdapter($sLogType,$oAdapter = null){
		...
		if($oAdapter instanceof \BoilerAppLogger\Service\Adapter\LogAdapterInterface)$this->adapters[$sLogType] = $oAdapter;
		elseif(is_string($oAdapter)){
			if(!class_exists($oAdapter))throw new \Exception('Undefined class name "'.$oAdapter.'"');
			elseif(!($oAdapter instanceof \BoilerAppLogger\Service\Adapter\LogAdapterInterface))throw new \Exception(sprintf(
				'Log adapter expects to be instance of \BoilerAppLogger\Service\Adapter\LogAdapterInterface; recieved "%s"',
				get_class($oAdapter)
			));
			$this->adapters[$sLogType] = new $oAdapter();
		}
		else throw new \Exception('Log adapter expects a string or be instance of \BoilerAppLogger\Service\Adapter\LogAdapterInterface; recieved "'.(is_object($oAdapter)?get_class($oAdapter):gettype($oAdapter)).'"');
		return $this;
	}
